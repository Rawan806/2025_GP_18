...
class _MatchResultsPageState extends State<MatchResultsPage> {
  ...
  Future<void> _fetchMatchesForReport() async {
    ...
    try {
      debugPrint('SmsService: Attempting to connect to AI backend at: ${widget.baseUrl}/search');
                      ...
                      onPressed: () async {
                        ...
                        final confirmed = await showDialog<bool>(
                          ...
                          builder: (_) => AlertDialog(
                            ...
                            content: Text(_tr('confirmMatchAndSendSMS')),
                            actions: [
                              TextButton(
                        ...
                        try {
                          ...
                          final locale = Localizations.localeOf(context);
                          final sentToUserStr = AppLocalizations.translate('sent_to_user', locale.languageCode);

                          await FirebaseFirestore.instance
                              .collection('foundItems')
                              .doc(widget.selectedDocId)
                              .update({
                            'status': sentToUserStr,
                          });
                          ...
                          if (lostDoc.exists) {
                            final userId = lostDoc.data()?['userId'];
                              ...
                              if (userDoc.exists) {
                                ...
                                if (phone != null) {
                                  ...
                                  final message = isArabic
                                      ? 'قد نكون وجدنا غرضك المفقود.\nيرجى التحقق من بلاغك في التطبيق وتأكيد ما إذا كان يخصك.\nتحتاج أيضًا إلى إضافة تفاصيل إضافية.'
                                      : 'We may have found your lost item.\nPlease check your report in the app and confirm if it is yours.\nYou also need to add extra details';
